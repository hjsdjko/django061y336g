...
<div :style='{"width":"80%","padding":"20px","margin":"10px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="成色" prop="chengse">
            <el-input v-model="ruleForm.chengse" 
                placeholder="成色" clearable :disabled=" false  ||ro.chengse"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="原价格" prop="yuanjiage">
...
<script>
  export default {
      ...
      init(type) {
        ...
        this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            ...
            if((json.zhihuanzhanghao!=''&&json.zhihuanzhanghao) || json.zhihuanzhanghao==0){
                this.ruleForm.zhihuanzhanghao = json.zhihuanzhanghao
            }
            if((json.zhihuanren!=''&&json.zhihuanren) || json.zhihuanren==0){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`xianzhishangpin/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('xianzhishangpin/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`xianzhishangpin/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
